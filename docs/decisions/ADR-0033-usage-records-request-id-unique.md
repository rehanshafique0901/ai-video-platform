...
**Status:** Accepted (Phase 3 W1.4, 2026-06-30). Validated end-to-end against Supabase Postgres 17.6 + pgvector 0.8.0; full 10-stage CI gate green locally (10/10 in 121.7s) and on GitHub Actions (1/1 check passed on PR #6, merged to `main` as commit `a7ab1ee`); alembic round-trip clean across both the W1.4-isolated path (`0006 → 0007 → 0006 → 0007`) and the full-baseline path (`0007 → empty → 0007` via CI gate stages 6 + 7, with all seven revisions `0001`–`0007` re-applying cleanly); schema validator 10/10 (the new `check_usage_records_per_partition_unique_indexes` reports `27/27 usage_records partition(s) carry uq_<child>_request_id with partial predicate WHERE (request_id IS NOT NULL)` — 26 monthly children + DEFAULT; documented-index count unchanged at 87 because per-child unique indexes are CI-visible via the new check rather than via `load_snapshot`'s bulk query, by deliberate design); ERD comparator 0 drift (51 entities and 60 shared design edges, both unchanged); pre-upgrade safety SELECT against the live target returned `dup_request_ids = 0`; `pg_indexes` shows 27 new partial-unique indexes named per the `uq_<child>_request_id` pattern present after upgrade and absent after downgrade; the parent's non-unique `ix_usage_records_request_id` propagating index is unaffected by both directions of the migration. First migration-coupled ADR to reference `docs/engineering/RUNBOOK_WAVE.md` in place of inlining operational steps (per `CONTRIBUTING.md` §6, established at `v0.3.3-infra`). Closes Phase 3 Wave 1.
