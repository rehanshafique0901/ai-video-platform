...
| Identity-Runtime authoring | ✅ | α10.0 | **fulfils the deferred *Identity-Runtime authoring* roadmap row** and lifts the α9.7 accepted limitation that v1 generation identity was a seed plus a global style. Governed by [`ADR-0055`](../decisions/ADR-0055-identity-runtime-authoring.md). The substrate already existed and was **unreachable**: the planner casts characters, the prompt builder composes their fragments, and the payload carries them — but nothing could author one, so every generation was about strangers. α10.0 supplies the missing surface and nothing else: **the planner, prompt builder, storyboard, shot-intent and resolver are byte-identical** (PF7 / IDENT-3). **A new owner-scoped bounded context** (`app/domain/identity_runtime/`, migration `0017` — four tables, profile root plus characters / locations / props) deliberately separate from the α2a *authentication* identity context, which owns `User`/`Tenant`/`Session` under the name this slice does not take. **Identity is Knowledge-plane state** (IDENT-4): a profile records what the creator declares exists and never accumulates execution outcomes, quality feedback, usage statistics or per-adapter tuning. **Binding is by snapshot at acceptance, never by reference** (IDENT-1): naming a world copies it whole into `generations.request`, so editing that world tomorrow cannot change what this generation executed or what it would replay as, and **deleting it leaves the past generation intact** — there is deliberately **no foreign key** from `generations.identity_id`, which is provenance carried as a value (ADR-0046 X5). **The payload version covers the whole envelope**: `SPEC_VERSION = 2` with nested identity, every v1 row still decodes to exactly what it always did, writers emit only v2, **no stored payload is ever rewritten and no legacy generation is given an identity**, and an undecodable payload **fails loudly** rather than guessing a world. **Identity is a third provenance class — a *request fact*** (D4), written once by ingress beside ownership and the persisted request, which **corrects a latent GEN-1 violation the grounding uncovered**: `identity_id` sat in the execution store's `ON CONFLICT DO UPDATE` enumeration while `begin()` always supplied `None`, so the first status write would have erased it the moment ingress started writing the column. **Only honourable state is authorable** (IDENT-2): reference images are refused with a 422 rather than accepted and discarded, because no registered adapter does reference conditioning — executable capability answers ***can*, never *should***, and never expresses quality, preference, routing, ranking or cost. **Additive apart from migration `0017`; no frozen path, no data migration, no resolver change**; new Stage 27 ([`PHASE3_ALPHA10_0_PREFLIGHT.md`](../engineering/PHASE3_ALPHA10_0_PREFLIGHT.md)). **Accepted limitations**: **one location per world** — multi-location worlds need per-shot casting, a Decision-plane change with its own contract, and the cap is that premise rather than a storage limit; **no per-shot casting** — the planner casts the whole world into every shot, unchanged; **reference images remain unauthorable** until an adapter with reference conditioning is registered; **hard delete is permitted** and leaves a dangling `identity_id` that no longer resolves, which is the snapshot guarantee working rather than a dangling reference; and **v1 payloads are still read by everything and written by nothing** — retirement is a separate decision with its own evidence, not a deadline |
...
- **IDENT-1** (α10.0) — **a generation is bound to the world it was accepted with, not to the world
  as it is now.** Naming a profile copies it whole into the request payload at acceptance; the run,
  every retry inside it, and every future replay see that snapshot and nothing else. Editing a
  profile is invisible to an accepted generation and deleting one leaves the record self-contained —
  which is why identity provenance is carried as a **value, never a foreign key** (ADR-0046 X5), and
  why editing and deleting are ordinary operations rather than history-altering ones (ADR-0055 D2).
- **IDENT-2** (α10.0) — **only honourable state is authorable.** No identity field is accepted
  through the API unless some path in the deployment consumes it; the failure mode being foreclosed
  is selling a creator a control that is silently discarded. Executable capability sets the boundary
  and **answers *can*, never *should***: it gates what may be offered and never expresses quality,
  preference, routing, ranking, cost, historical success, or a planner decision (ADR-0055 D5 — the
  DISP-1 principle applied one layer up, to authoring).
- **IDENT-3** (α10.0) — **authoring supplies world state, never policy.** Shot casting, ordering,
  routing, preference and decomposition remain the Decision plane's. Identity never becomes a lever
  over them and never becomes an input to resolution: `resolve()` keeps its four inputs and gains
  nothing from a profile (ADR-0055 D5).
- **IDENT-4** (α10.0) — **identity is knowledge, never measurement.** A profile records what the
  creator declares exists; it never accumulates execution outcomes, quality feedback, usage
  statistics or per-adapter tuning, and no Decision-plane behaviour may become a function of such
  data. This forecloses the most plausible future coupling between Execution results and Decision
  inputs — "this character usually renders better on X" is exactly the leak it prevents
  (ADR-0055 D1/IDENT-4, extending F4/F5 to creator knowledge).
...
> α9.6 TikTok destination adapter, the α9.7 generation ingress, the
> α9.8 worker runtime host, and the α10.0 Identity-Runtime authoring) have
...
| **Per-shot casting** | α10.0 ships **whole-world casting** — the planner casts every authored character and the single location into all six shots, unchanged. Deciding *which* character appears in *which* shot is a Decision-plane change with its own contract, and it is the premise the one-location cap rests on: multi-location worlds become meaningful only once a shot can be cast individually |
| **Reference-image conditioning** | Reference images stay **unauthorable** by ADR-0055 D5: no registered adapter does reference conditioning, and accepting a control the deployment discards is the defect IDENT-2 forecloses. Registering such an adapter is what unlocks the field — the adapter comes first, the authoring surface second |
