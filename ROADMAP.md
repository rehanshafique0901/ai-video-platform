...
| Wave | Theme | Items |
|---|---|---|
| **W1** | Schema integrity (correctness) | **W1.1 ✅ Complete (ADR-0030, migration `0003_export_jobs_partial_unique`, 2026-06-29)** — `export_jobs (render_job_id, format, quality, orientation)` partial-unique DB constraint promotion. W1.2 idempotency_keys CHECK+updated_at, W1.3 distributed_locks lease CHECK, W1.4 usage_records per-partition request_id uniqueness — each its own PR + ADR. |
