...
| Date | Change |
|---|---|
| 2026-07-25 | **Phase 3 (Seed round-trip verification) implemented + proven on live PostgreSQL** — `backend/scripts/verify_seed_roundtrip.py` runs the acceptance loop (empty→seed creates all 60 rows; re-seed → 0 writes + digest/revision unchanged; single manifest change → exactly 1 row updates; removed provider → entity `enabled=false` + derived edge deleted, nothing hard-deleted; registry metadata updated). Wired into `ci_gate.py` as **stage 11** (`requires_db`, appended after coverage so the destructive-migration guard on stages 5–9/6 is untouched); `live_selected` now derives from `requires_db`. Verified end-to-end against the live PostgreSQL: `0010` upgrade→downgrade→upgrade clean, then all round-trip checks green; shared DB restored to `0009` afterwards. **Regression fixed:** the diff now canonicalises `Decimal` (DB `Numeric` round-trip) against manifest floats, so cost-bearing adapters no longer show as spurious updates (locked by a unit test). Seeder unit tests: 12. |
| 2026-07-25 | **Phase 2 (Seeder) implemented** — `backend/scripts/seed_providers.py`: validate → SHA-256 digest of the canonicalised manifests → short-circuit on unchanged digest → idempotent natural-key upsert in dependency order → removed entities soft-disabled (`enabled=false`, never deleted) → provenance written (incl. new `catalogue_version` column added to `provider_registry_meta` in `0010`). Deterministic plan+digest layer unit-tested (10 cases). **Boundary:** the seeder establishes catalogue *state* only; no runtime component consumes catalogue data until α8.5e. **Two decisions ratified by review:** (a) removed *derived edges* (`capability_dependencies`, `adapter_fallbacks`) are **synced/deleted**, not soft-disabled — entity tables are never deleted, relationship tables regenerate from the manifest; (b) `enabled=false` (operational availability) is kept orthogonal to `status` (implementation lifecycle) — no `deprecated` enum value is introduced until a real runtime requirement exists. **Deferred capability (recorded):** variant→`ai_models` flattening (D-F / W8.5d.6) is **deferred** — `0010` created no variants/models table, the runtime already reads flat `provider_adapters` with no inheritance (so W8.5d.6's "no runtime inheritance" intent holds), and faithful flattening needs concrete variant metadata + a model-catalogue consumer. Re-introduce with α8.5e/model-catalogue when there is an actual selector, not speculatively. |
