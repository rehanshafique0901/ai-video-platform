...
| 3 | Authentication | **🟦 IN PROGRESS** — α1 (arch bootstrap) ✅ 2026-06-30 (v0.4.0), α2a (auth: register + login) ✅ 2026-07-01 (v0.4.1), α2b (refresh + logout) ✅ 2026-07-01 (v0.4.2), α3 (`GET /users/me` read + `CurrentUserDep`) ✅ (v0.4.3), α4 (`PATCH /users/me` mutation) ✅ 2026-07-10 (v0.4.4), α5a (Projects create + read) ✅ 2026-07-11 (v0.4.5), α5b (Projects update + soft-delete) ✅ 2026-07-11 (v0.4.6), α5c (Scenes CRUD + reorder) ✅ 2026-07-11 (v0.4.7), α5d.1 (Project Versions capture + read) ✅ 2026-07-12 (v0.4.8), α5d.2 (Project Version restore + diff; Aggregate OCC Rule) ✅ 2026-07-12 (v0.4.9), α5d.3 (Project Version branch — fork to a new project) ✅ 2026-07-12 (v0.4.10), α6.1 (Prompt aggregate — generation-input CRUD) ✅ 2026-07-12 (v0.4.11), α6.2 (Media Asset aggregate — generation-output CRUD) ✅ 2026-07-13 (v0.4.12), α6.3a (Timeline aggregate — root + tracks; self-contained OCC via `timelines.version`, ADR-0038) ✅ 2026-07-13 (v0.4.13), α6.3b (Timeline aggregate — clips; children of the timeline OCC, `media_asset_id` validation, immutable `track_id`) ✅ 2026-07-14 (v0.4.14), α7.1 (RenderJob aggregate — the first orchestration slice; self-versioned OCC via `render_jobs.version`, status machine + version-fenced cancel, idempotent create, `RenderJobCreated`/`RenderJobCanceled` outbox events per D9; NO worker — ADR-0039) ✅ 2026-07-16 (v0.4.15), α7.2 (WorkflowRun aggregate + synchronous deterministic runner — the first sequencing orchestration slice; status-guarded CAS (no `version` column, a deliberate divergence from α7.1), ordered `WorkflowStep` + append-only `WorkflowCheckpoint` children, in-code workflow registry of pure/deterministic/side-effect-free step handlers (D3.11), resume-safe `advance` + deterministic retry accounting, idempotent create, six `WorkflowRun*` outbox events per D9; NO worker/providers/scheduler — ADR-0040) ✅ 2026-07-16 (v0.4.16), α7.3 (Outbox relay + distributed lock manager — pure execution-substrate infrastructure; library-only `RelayService.relay_once() -> RelayResult` draining the `event_outbox` through a `PublisherPort` (default synchronous in-process publisher, no `event_log` projection), poison events parked in-place after `max_attempts` with an ERROR structured log (no DLQ/scheduler), owner-fenced `SqlAlchemyDistributedLockManager` with steal-after-expiry + explicit `reclaim_expired()` over the baseline `distributed_locks` table per ADR-0032; NO worker/daemon/CLI/HTTP/broker/Celery/Redis — the worker loop is α8.1) ✅ 2026-07-17 (v0.4.17), α7.4 (Provider Skeleton — the provider abstraction layer; four async capability ports (LLM/Image/Video/Voice), a framework-free registry with explicit registration + capability discovery, a `StepCommandDispatcher` mapping `StepCommand`s to capability calls (closed table, four kinds; render/export/storage excluded), typed provider-error hierarchy, immutable `ProviderMetadata`, one deterministic mock per capability (video models the async `IN_PROGRESS` + `provider_job_id` path), minimal read-only `provider_settings` seam (tenant-shadows-global), `import-linter` strict-leaf on `infrastructure.ai.providers`; NO HTTP/API keys/external calls/Redis/Celery/retries/fallback/usage/events/polling/webhooks — pure architecture, runner untouched, zero migration) ✅ 2026-07-17 (v0.4.18) |
