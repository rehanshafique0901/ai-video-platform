...
**Status:** Accepted (Phase 3 W1.2, 2026-06-29).
...
**Status:** Proposed (Phase 3 W1.4, 2026-06-30).
**File:** [`docs/decisions/ADR-0033-usage-records-request-id-unique.md`](docs/decisions/ADR-0033-usage-records-request-id-unique.md)
**Summary:** Resolves `docs/database/schema.md` §37 Q6 by promoting per-partition partial-unique `(request_id) WHERE request_id IS NOT NULL` indexes named `uq_<child>_request_id` to every child partition of `usage_records` (26 monthly + DEFAULT today). Single-column by deliberate choice — the Phase 3 wave-planning artifacts (ROADMAP, INDEX_STRATEGY, ADR-0031 line 218, the v0.3.3-infra `VARCHAR(255)` widening explicitly scoped for the 35-char `0007_usage_records_request_id_unique` slug) consistently anticipate this shape. Earlier architectural documents (`schema.md` §18 Step-A draft, §31 line 1175, `API_CONTRACT.md` line 233, `ARCHITECTURE.md` §8k.1) describe a broader provider-scoped `(provider, request_id)` idempotency pattern at the application/architectural layer; W1.4 implements the scope reflected in the Phase 3 planning artifacts without attempting to reconcile that broader architectural question. The per-child mechanic is PostgreSQL's standard pattern for unique-on-non-partition-key constraints (the parent-level form is rejected because the unique key omits `occurred_at`) — not a workaround. The ORM declaration is intentionally absent for the same reason; CI visibility for the per-child indexes is provided by `backend/scripts/validate_schema.py::check_usage_records_per_partition_unique_indexes`, a tooling addition compensating for the bulk-snapshot performance optimisation that elides partition children. First migration-coupled ADR to reference `docs/engineering/RUNBOOK_WAVE.md` in place of inlining operational steps. Wave 1 of Phase 3 closes with this ADR. See the ADR file for full Context (including the architectural-review process preceding this ADR), Decision, 7 Alternatives Considered (including the rejected `(provider, request_id)` scope expansion and `(model_id, request_id)` invention), Migration Plan, 3-tier Rollback, Consequences, Implementation Notes (per-child mechanic, ORM-absence rationale, validator extension as CI visibility, future-partition contract), Future Considerations (provider-scoped DB-level enforcement reserved for a separate decision informed by CR-12 implementation evidence), and 22-item Acceptance Criteria.
