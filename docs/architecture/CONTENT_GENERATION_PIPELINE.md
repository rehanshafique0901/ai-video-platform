...
- **Config precedence** (schema §27.3): `provider_settings` tenant row →
  ...
  ciphertext at rest.
- **Costing:** every provider call is wrapped by the Usage Recorder
  (`infrastructure/ai/middleware/usage_recorder.py`, CR-12) which writes one
  `usage_records` row priced against `ai_model_pricing`, and is **idempotent on
  `(provider, request_id)`** (schema §31 note).
