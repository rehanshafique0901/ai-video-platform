...
| **α8.4b** | FFmpeg render engine | RenderJob worker transitions (`running/succeeded/failed`, progress, `output_media_asset_id`) + compose; transitions/effects (α6.4 write paths first); duration/dimensions/codec/thumbnails/previews on generated media — ADR-0041 D12 | none |
