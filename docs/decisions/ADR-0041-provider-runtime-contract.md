...
| 2026-07-25 | **α8.5c addendum — capability-first Capability Catalogue & Provider Registry (design-time spec, tooling-only, no version bump).** Records the direction for the α7.4-deferred selection precedence (D2): provider truth becomes **capability → providers**, authored as a human-reviewable **design-time YAML spec** under `backend/providers/` (`capabilities.yaml` + `registry.yaml`), validated by an **offline** CI validator (`scripts/validate_providers.py`, gate **Stage 0**) against a strict Pydantic schema (`scripts/provider_manifest.py`). The spec is **two-level** — a coarse `kind` (`llm\|image\|video\|voice`, mirroring `plugin_kind_enum` / the code `Capability` enum) plus an open **fine `capability`** vocabulary (27 terms) that maps onto the existing `capabilities text[]` columns — so the catalogue expands **without a migration or an enum change**. Selection is **score + routing-strategy** driven (`quality/cost/speed/reliability`, `free_first`/`lowest_cost`/`highest_quality`/…); **integer priority is removed entirely**. Static declared metadata (scores/license/free-tier/adapter/family) lives in Git; operational state (health/latency/quota/success-rate) stays in the DB. **Adapter IDs are first-class** (the unit the runtime will load); families carry variants with acyclic inheritance. Six invariants **W8.5c.1–6** (capability-first selection · YAML is design-time only, DB is the runtime SoT — the **runtime never reads YAML** · static/operational separation · AI vs publishing registry separation · offline validation · anti-drift). **Scope is tooling + spec only** (S-A1): no runtime code, no seeder, no migration. Follow-ups: **α8.5d** (YAML → validator → seeder → DB, the runtime source of truth) then the resolver (this ADR's deferred D2 precedence, reading operational state from the DB). Publishing destinations get a **parallel** registry with **shared** tooling (α8.6). See `docs/engineering/PHASE3_ALPHA8_5c_PREFLIGHT.md`. |
