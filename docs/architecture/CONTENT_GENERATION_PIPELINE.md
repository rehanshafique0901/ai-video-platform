...
the α8.5b.3 notification projection, a future publish step) subscribes to events
without touching the producing aggregate. It is the same one-way-dependency rule the CI `import-linter`
...
| **α8.5b.3** ✅ | Notifications — project export terminal events into in-app notifications | `NotificationProjection` (2nd relay consumer on the existing `PublisherPort`) reacts to `ExportJobSucceeded`/`ExportJobFailed` → `CreateNotification` writes one `notifications` row per recipient. Exactly-once per recipient per source event, DB-enforced via new nullable `source_event_id` + partial `UNIQUE (user_id, source_event_id)` (Ruling D). **W8.5b.6–W8.5b.7**. Additive consumer (no frozen contract touched). Read API → α8.5b.3r, email → α8.5b.4. **One additive migration** (`0009`) — shipped `v0.4.33` | one |
| **α8.5b.3r** ✅ | Notification read API — read & manage the projection | Owner-facing read-model completion on the α8.5b.3 projection: `GET /notifications` (keyset feed — reuses the α5a pagination primitive), `GET /notifications/unread-count`, `POST /notifications/{id}/read` + `/read-all` (action verbs). Pure additive repo methods (`list_for_user`/`count_unread`/`mark_read`/`mark_all_read`); read-state metadata-only + order-stable (**W8.5b.8–W8.5b.10**); Gate 1/2 PASS (query-only; projection contract intact). Archive/inbox features out; email → α8.5b.4. **Zero migration** — shipped `v0.4.34` | none |
| **α8.5c** | Capability Catalogue & Provider Registry (tooling) | **design-time spec, tooling-only** — three manifests under `backend/providers/` (`capabilities.yaml` vocabulary + `providers.yaml` + `routing.yaml`), a strict Pydantic schema, an **offline validator** (`scripts/validate_providers.py`) and CI **Stage 0**. Capability-first (*capability → providers*); score+strategy (no priority); static-only (operational state stays in the DB); the **runtime never reads the YAML** (W8.5c.2). Publishing excluded (separate registry, α8.6). No runtime, no migration, no version bump — see `docs/engineering/PHASE3_ALPHA8_5c_PREFLIGHT.md` | none |
| **α8.5x** ✅gov | **AI Runtime & Generation Architecture (governance — Accepted)** | **ADR-0044** (Accepted, docs-only) locks the verify-driven, capability-first, **local-first** pipeline (Plan → Generate → Verify → Repair). Requirement set **AR1–AR18** with invariants **W8.5x.1–W8.5x.18**; local execution/hardware abstraction + recommended local model defaults; freeze/boundary compliance via the two gates (Gate 1 additive to ADR-0042, Gate 2 respects ADR-0043). Carves out the **Minimum Runtime Contract MRC-1…MRC-8** as the Phase-1 production scope (the only runtime α8.5d/α8.5e + the first generation slice must ship); Phase 2 defers the AR depth. Governs α8.5d→α8.6. **No code/migration/version bump** | none |
| **α8.5d** | Seed — YAML → DB (α8.5x) | validator → **seeder + additive migration** seeds the α8.5c spec into the DB, making the DB the *populated* runtime source of truth; carries **execution/hardware/mode metadata** in the provider/runtime schema (ADR-0044 X-G) | additive |
| **α8.5e** | Capability resolver — MRC-2/-3 (α8.5x) | realises ADR-0041 **D2** deferred precedence: capability + active strategy → provider/adapter, **local-first / free-first** (AR7/AR8), reading static + operational state from the DB; layered into `ProviderRegistry.resolve()` without changing callers | none |
| **α8.5x-mrc** | **MRC — the MVP** (α8.5x) | the Phase-1 runtime slice(s): MRC-1 planner (prompt → structured `Project`, AR1) → MRC-4 character memory (`CharacterSheet`: seed/refs/style/voice, AR2/AR16) → MRC-5 scene-by-scene (AR13) → generate → MRC-6 threshold CLIP/face/prompt verification (AR4 thin; regenerate-only) → MRC-7 resume-from-scene (AR14/AR17) → MRC-8 asset cache (AR6/AR16) → render → export, capturing **AR18 provenance** throughout. Planner upstream, verify/repair downstream — Gate 1 = No frozen change | additive |
| **α8.5x-phase2** | Deferred AR depth (α8.5x, D8) | advanced/bounded repair strategies (AR5), multiple planners, quality modes (AR11 full), multi-agent verification (AR4 depth), GPU orchestration + Apple-Silicon tuning (AR9/AR10), adaptive routing, distributed rendering, full project-memory aggregate (AR16). Non-blocking for MVP | additive |
| **α8.6** | Publishing | `PublishJob` + `SocialAccount` + destination OAuth — a **new bounded context** (destinations are not AI providers) with its **own** parallel registry (shared α8.5c tooling); sequenced **after** the AR runtime (ADR-0044); needs a migration | none |
...
| 2026-07-24 | **α8.5b.3 Notifications shipped** (`v0.4.33`; **third + final distribution-stage slice**). Closes the loop the export/delivery slices opened: the artifact now *exists* (α8.5a), is *obtainable* (α8.5b.1) from *any backend* (α8.5b.2), and the requester is now *told* when it's ready or failed. Two gates in the pre-flight — Gate 1 (frozen orchestration surface? **No** — additive consumer) and Gate 2 (ADR-0043: far below the render boundary). The entire runtime addition is **one downstream projection** — a second, fully independent consumer registered on the existing in-process `PublisherPort` in the composition root, proving the platform's fan-out seam (the runner knows nothing about it, kin to α8.4a ingestion). **`NotificationProjection`** (`app/application/use_cases/notifications/notification_projection.py` — **Ruling C**: a relay subscriber, deliberately named a *projection* not a service because it *derives read state from immutable events* and never orchestrates) reacts to **`ExportJobSucceeded` + `ExportJobFailed` only** (**Ruling B**; recipient read straight from the event's `requested_by_user_id`, no ownership lookup), maps each to notification content (kind/title/body/payload), and delegates to a fresh **`CreateNotification`** (`create_notification.py`, own UoW per event) which persists one `notifications` row (stamping `delivered_in_app_at`). **Exactly-once = at-least-once relay + a DB uniqueness invariant** (**Ruling D**): a new nullable **`notifications.source_event_id`** (the outbox `event.id`; **no FK** to `event_outbox` — transport state vs product state) + a **partial** `UNIQUE (user_id, source_event_id) WHERE source_event_id IS NOT NULL`; **`NotificationRepository`** maps the violation → `ConflictError`, and the use case treats it as an already-notified **no-op**. `(user_id, source_event_id)` (not `source_event_id` alone) future-proofs fan-out while keeping today's single-recipient semantics. Two new invariants: **W8.5b.6** — notification creation is a **pure projection** of immutable events (never mutates export/render/orchestration state, re-drives the export, or calls back into the frozen pipeline); **W8.5b.7** — a notification is projected **exactly once per recipient per source event, enforced by the persistence layer, not by subscriber control flow**. Read/query API (list/unread/mark-read/archive) deferred to **α8.5b.3r**; email/push/websocket → **α8.5b.4**; publishing → **α8.6**. **Freeze guard green, zero override markers — no frozen orchestration path changed.** **One small additive migration** (`0009` — a nullable column + a partial unique index that *encodes* the exactly-once invariant, not a new capability). Next slice: α8.5b.3r (notifications read API) / α8.6 (publishing), or α8.4f (composition, after α6.4 authoring). |
| 2026-07-25 | **α8.5x ADR-0044 Accepted + MRC carved out** (governance, docs-only). Rulings signed off (forks X-A…X-G all approved; publishing stays after the runtime; additive/zero-override freeze posture; execution/hardware/mode metadata folded into the α8.5d provider/runtime schema). Added **AR16 Project Memory**, **AR17 Workflow Checkpoints**, **AR18 Provider Transparency** (invariants extended to **W8.5x.1–W8.5x.18**). The load-bearing scope ruling: AR1–AR18 are the *long-term* architecture; a **Minimum Runtime Contract MRC-1…MRC-8** is the *only* runtime α8.5d/α8.5e + the first generation slice must ship for a usable 1–2-day MVP (planner → character memory → scene-by-scene → generate → threshold verification → resume → asset cache → render → export, capturing AR18 provenance from day one). **Phase 2** defers advanced repair, multiple planners, quality modes, multi-agent verification, GPU orchestration, adaptive routing, distributed rendering, and the full project-memory aggregate. MRC-first merge order: chore → α8.5c → ADR-0044 → α8.5d → α8.5e → α8.5x-mrc → α8.6 → Phase 2/UI. Next: implement α8.5d (seed). |
| 2026-07-25 | **α8.5x AI Runtime & Generation Architecture locked** (governance, **ADR-0044**, docs-only — no code/migration/version bump). After α8.5c inverted provider truth to *capability → providers* (design-time spec + offline validator + CI Stage 0), the next step is the *runtime that plans, selects, verifies, and repairs* generation — which touches every future provider, so its contract is locked **before** α8.5d/α8.5e/α8.6. ADR-0044 fixes the **verify-driven, capability-first, local-first** pipeline (Plan → Generate → Verify → Repair; *the AI never trusts itself*), the requirement set **AR1–AR15** each with an enforceable invariant **W8.5x.1–W8.5x.15** (planning-before-generation, character-identity lock, scene state, visual verification, bounded repair loop, asset reuse, local-first preference, provider scoring, Intel/Apple hardware optimization, generation modes, one-module-one-job, long-video scene decomposition, resume-from-last-verified-scene, cost optimizer), the **local execution/hardware abstraction** (`IExecutionEnvironment` + local adapters in the capability registry) with recommended local model defaults (Ollama/Qwen/Gemma planners, SDXL/FLUX-schnell images, Real-ESRGAN/RIFE/CodeFormer, Kokoro TTS, CLIP/SigLIP verification), and **freeze/boundary compliance** via the two gates — planner *upstream* of the frozen runner, verify/repair *downstream*, resolver = the ADR-0041 D2 designed `resolve()` seam, execution = new adapters (Gate 1 = No frozen change; Gate 2 = render boundary intact). Sequences the program **α8.5d** (seed YAML→DB) → **α8.5e** (local/free-first resolver) → plan/identity/scene → verify/repair/reuse → exec → **α8.6** publishing → UI, with a **thin-vertical-slice MVP** (a full AR1–AR15 build is a multi-slice program, not a 1–2-day effort at this quality bar). Forks **X-A…X-G** raised for sign-off. Builds on ADR-0037/0038/0039/0040/0041/0042/0043 + α8.5c. Next: sign off ADR-0044 forks, then implement α8.5d. |
| 2026-07-24 | **α8.5b.3r Notification read API shipped** (`v0.4.34`; the read/query completion deferred from α8.5b.3). α8.5b.3 established `Immutable Event → Exactly-once Projection → Notification row`; this slice adds the owner-facing surface on top of that stable foundation and **nothing more**. Two gates in the pre-flight — **Gate 1** (frozen orchestration surface? **No** — read-model only) and **Gate 2** (projection contract intact — only exposes/updates metadata, never changes how notifications are projected/created). Four endpoints: **`GET /notifications`** (keyset feed, newest-first, owner-scoped — **Fork A/B**: reuses `app/application/pagination.py` verbatim, the α5a `GET /projects` shape; no offset / no notification-specific cursor), **`GET /notifications/unread-count`** → `{count}` (index-only scan of `ix_notifications_user_id_unread`), **`POST /notifications/{id}/read`** + **`POST /notifications/read-all`** → `{updated}` (**Fork C** — action verbs matching `…/render-jobs/{id}/cancel`, not `PATCH`). Repository additions are **pure additive** (**Fork D** — `list_for_user`/`count_unread`/`mark_read`/`mark_all_read` on the same `INotificationRepository`; `add` + `NotificationProjection` byte-for-byte unchanged); `mark_read` is an owner-scoped CAS on `read_at IS NULL` (already-read → unchanged row so mark-read is idempotent; foreign/missing → `None` → 404). The domain **`Notification`** entity gained `read_at`/`archived` (now legitimate domain state, not repo-only details). Three new invariants: **W8.5b.8** — queries never expose another principal's notifications (owner-scoped, uniform 404); **W8.5b.9** — read-state mutations write **only** `read_at`, never identity/`source_event_id`/delivery provenance; **W8.5b.10** — ordering is **observational only** (`created_at DESC, id DESC`, independent of `read_at`), so mark-read/mark-all never reshuffle the feed. **Fork E — existing index, zero migration**: the composite keyset index `(user_id, created_at, id)` is **intentionally deferred pending observed production need** (optimize when justified, not speculatively). Archive/delete/search/filters/folders/labels/pinning/snooze **out** (read-model completion, not an inbox product); email → **α8.5b.4**. **Freeze guard green, zero override markers — no frozen orchestration path changed.** **Zero migration** (`read_at`/`archived`/both feed indexes predate the slice). Next slice: α8.5b.4 (email channel) / α8.6 (publishing), or α8.4f (composition, after α6.4 authoring). |
