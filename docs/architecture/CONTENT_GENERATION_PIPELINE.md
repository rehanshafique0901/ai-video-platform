...
| **α8.5x** | **AI Runtime & Generation Architecture (governance)** | **ADR-0044** locks the verify-driven, capability-first, **local-first** generation pipeline (Plan → Generate → Verify → Repair). Requirement set **AR1–AR15** with invariants **W8.5x.1–W8.5x.15**; local execution/hardware abstraction + recommended local model defaults; freeze/boundary compliance via the two gates (Gate 1 additive to ADR-0042, Gate 2 respects ADR-0043). Governs α8.5d→α8.6. **Docs-only, no code/migration/version bump** | none |
| **α8.5d** | Seed — YAML → DB (α8.5x) | validator → **seeder + additive migration** seeds the α8.5c spec into the DB, making the DB the *populated* runtime source of truth (adds execution/mode metadata per ADR-0044 X-G) | additive |
| **α8.5e** | Capability resolver (α8.5x) | realises ADR-0041 **D2** deferred precedence: capability + active strategy → provider/adapter, **local-first / free-first** (AR7/AR8), reading static + operational state from the DB; layered into `ProviderRegistry.resolve()` without changing callers | none |
| **α8.5x-runtime** | Planner / identity / scene / verify / repair / reuse / exec (α8.5x) | additive bounded contexts realising AR1–AR15: `GenerationPlan` (AR1/AR11/AR15), `CharacterSheet` (AR2), `Scene` state + long-video decomposition + resume (AR3/AR13/AR14), `IVerifier` + bounded repair loop (AR4/AR5), asset-reuse index (AR6), `IExecutionEnvironment` + local adapters Intel-first/Apple-Silicon-later (AR9/AR10). Planner upstream, verify/repair downstream — Gate 1 = No frozen change | additive |
