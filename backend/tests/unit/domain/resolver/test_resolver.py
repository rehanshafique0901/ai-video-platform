...
Coverage map:
  ...
  H  executability (ADR-0054 D1): the deployment-scoped input, checked first

Tests call ``_resolve``, which supplies "everything in this catalogue is constructible"
unless the test is specifically about executability. Section H passes the set explicitly.
"""
...
def _resolve(
    ...
    rt: RuntimeSnapshot,
    executable: ExecutableAdapters | None = None,
) -> Resolution:
    return resolve(request, cat, rt, executable or f.all_executable(cat))
...
def test_a1_balanced_orders_by_score_desc() -> None:
    res = _resolve(_req(), f.catalogue(), f.runtime())
...
def test_a2_ineligible_come_after_eligible() -> None:
    ...
    res = _resolve(_req(local_only=True), f.catalogue(), f.runtime())
...
def test_b1_same_inputs_identical_output() -> None:
    ...
    assert _resolve(_req(), cat, rt) == _resolve(_req(), cat, rt)


def test_b2_fingerprint_stable_and_request_sensitive() -> None:
    a = _resolve(_req(prompt="cat"), f.catalogue(), f.runtime())
    b = _resolve(_req(prompt="cat"), f.catalogue(), f.runtime())
    c = _resolve(_req(prompt="dog"), f.catalogue(), f.runtime())
...
def test_c1_equal_score_breaks_by_reliability_then_id() -> None:
    ...
    res = _resolve(_req(), f.catalogue(providers=providers, adapters=adapters), f.runtime())
...
def test_d1_privacy_mode_excludes_cloud() -> None:
    res = _resolve(_req(privacy_mode=True), f.catalogue(), f.runtime())
...
def test_d2b_paid_providers_not_allowed() -> None:
    ...
    res = _resolve(_req(allow_paid_providers=False), f.catalogue(), f.runtime())
...
def test_d3_budget_zero_excludes_paid() -> None:
    res = _resolve(_req(budget=0), f.catalogue(), f.runtime())
    assert _reason(res, "fal.flux") == "budget_zero_paid"
    assert {c.provider_id for c in res.eligible} == {"pollinations", "comfyui"}


def test_d4_over_budget_excludes_expensive_paid() -> None:
    res = _resolve(_req(budget=0.005), f.catalogue(), f.runtime())  # fal costs 0.01
...
def test_d5_quota_exhausted() -> None:
    ...
    res = _resolve(_req(), f.catalogue(), rt)
...
def test_d7_insufficient_hardware() -> None:
    res = _resolve(_req(device="small_box"), f.catalogue(), f.runtime())  # 8GB < min 16
...
def test_d9_unknown_provider_is_ineligible_not_crash() -> None:
    ...
    res = _resolve(_req(), f.catalogue(providers={}, adapters=adapters), f.runtime())
...
def test_e1_free_first_ranks_free_top_and_paid_last() -> None:
    ...
    res = _resolve(_req(), cat, f.runtime())
...
def test_e3_per_capability_routing_overrides_default() -> None:
    ...
    assert _resolve(_req(), cat, f.runtime()).routing_strategy == RoutingStrategy.HIGHEST_QUALITY
...
def test_g3_provenance_fields_populated() -> None:
    res = _resolve(_req(), f.catalogue(version="2026.07", digest="abc123"), f.runtime())
...
# --------------------------------------------------------------------------- #
# H — executability (ADR-0054 D1)
# --------------------------------------------------------------------------- #
def test_h1_non_executable_adapter_is_ineligible_with_reason() -> None:
    cat = f.catalogue()
    res = _resolve(_req(), cat, f.runtime(), f.executable("pollinations.image"))
    assert [c.adapter_id for c in res.eligible] == ["pollinations.image"]
    assert _reason(res, "comfyui.flux_schnell") == "not_executable"
    assert _reason(res, "fal.flux") == "not_executable"


def test_h2_non_executable_adapters_are_still_returned_and_explained() -> None:
    # DISP-1: an exclusion is recorded, never silently dropped — that is what lets the
    # ledger explain the decision without persisting the executable set separately.
    cat = f.catalogue()
    res = _resolve(_req(), cat, f.runtime(), f.executable())
    assert res.top is None
    assert {c.adapter_id for c in res.candidates} == {a.id for a in cat.adapters}
    assert {c.ineligible_reason for c in res.candidates} == {"not_executable"}


def test_h3_executability_is_reported_ahead_of_every_other_reason() -> None:
    # The ordering is load-bearing: only the first failing constraint is reported, so a
    # candidate that also fails a later check must still read `not_executable`.
    providers = {"off": f.provider("off", enabled=False, pricing=Pricing.PAID)}
    adapters = (f.adapter("off.a", "off", mode=ExecutionMode.CLOUD, enabled=False),)
    cat = f.catalogue(providers=providers, adapters=adapters)
    rt = f.runtime(
        health={"off": ProviderHealth(provider_id="off", health_score=0.0)},
        quota={"off": (QuotaState(provider_id="off", window="daily", remaining=0),)},
    )
    res = _resolve(_req(budget=0, privacy_mode=True), cat, rt, f.executable())
    assert _reason(res, "off.a") == "not_executable"

    # …and the later reason surfaces once the adapter becomes executable.
    res = _resolve(_req(budget=0, privacy_mode=True), cat, rt, f.executable("off.a"))
    assert _reason(res, "off.a") == "provider_disabled"


def test_h4_executable_set_does_not_change_the_manifest_digest() -> None:
    # D1: executability is deployment-derived, so it must not leak into catalogue identity.
    cat = f.catalogue(digest="abc123")
    wide = _resolve(_req(), cat, f.runtime(), f.all_executable(cat))
    narrow = _resolve(_req(), cat, f.runtime(), f.executable("pollinations.image"))
    assert wide.manifest_digest == narrow.manifest_digest == "abc123"
    assert wide.catalogue_version == narrow.catalogue_version


def test_h5_empty_executable_set_fails_closed() -> None:
    res = _resolve(_req(), f.catalogue(), f.runtime(), ExecutableAdapters())
    assert res.top is None
    assert res.eligible == ()
