"""``DeleteClip`` use case (Slice α6.3b).

Contract (API_CONTRACT §3.2.4):

    DELETE /api/v1/projects/{project_id}/timeline/tracks/{track_id}/clips/{clip_id}?version=<n>
      → 204  (no body)                                  (soft-deleted; aggregate bumped)
      → 404  { error: { code: NOT_FOUND, ... } }        (project/timeline/track/clip missing / not yours / already deleted)
      → 412  { error: { code: VERSION_CONFLICT, ... } } (stale timeline version)
      → 401  { error: { code: UNAUTHENTICATED, ... } }  (via CurrentUserDep)

Deleting a clip changes the aggregate, so it is fenced on — and advances —
``timelines.version`` (ADR-0038 / Q13); the expected version is **required** (a
``version`` query parameter, mirroring ``DELETE …/tracks/{id}``, Q3). Control flow
is **404-before-412**: a missing / already-deleted clip is a uniform ``404``
(idempotent-by-404) *before* the fence is consulted, so a repeat delete is
``404`` (not ``412``). Only a live clip with a stale token yields ``412``. Does
**not** touch ``projects.version``.
"""
...
class DeleteClip:
    """Version-fenced (on the timeline) soft-delete of a clip; 404 otherwise."""
    ...
    async def execute(
        ...
        *,
        ...
        async with self._uow:
            ...
            # 404-before-412: an absent/already-deleted clip is a 404 regardless
            ...
            deleted = await self._uow.timeline.soft_delete_clip(track_id, clip_id)
            if not deleted:
