...
- **Costing:** every **terminal** provider call is wrapped by the Usage Recorder
  ...
  (ADR-0033 per-partition unique index; a replay returns the existing row). Shipped
  as a **seam** the α7.6 pipeline calls around the dispatch. Scoped for α7.5: the
