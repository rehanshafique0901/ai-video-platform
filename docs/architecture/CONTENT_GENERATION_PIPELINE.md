...
| **α8.3b** | Webhook completion ingress | inbound `/webhooks/providers/{name}` as a **thin second ingress** to the same `complete()` — signature verification + `idempotency_keys(resource_type='webhook')` — ADR-0041 D7 | none |
