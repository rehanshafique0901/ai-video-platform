"""Shared scaffolding for the α5c Scene use-case unit tests.

Every scene use case runs the **two-level visibility gate** (α5c D6): it
first calls ``projects.get_owned`` and 404s if the caller does not own a
live project, then reaches the scene port. So every test needs a seeded,
owned :class:`Project` in the ``FakeProjectRepository``. ``build_env``
wires that up and returns the fakes so tests can assert on repo state and
``commit`` bookkeeping.
"""
...
def build_env() -> Env:
    """Wire a UoW with one owned live project and an empty scene repo."""
    owner_user_id = uuid4()
    tenant_id = uuid4()
